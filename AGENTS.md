...
- 编译器和工程路径定义在 `.Codex/skills/keil-build/SKILL.md` 中
...
- **用户自由文件**（CubeMX 不触碰）：
  ...
  - `src/` — 用户代码（`app/`、`service/`、`driver/`、`bsp/`、`lib/`）；`Core/` 只留 CubeMX 生成代码
...
src/            ← 用户代码（app 产品 / service 领域 / driver 外设 / bsp 板级 / lib 第三方与算法）
